...
if __name__ == "__main__":
    print("=" * 55)
    print("  TEST MANDIRI — modules/graph_rujukan.py")
    print("=" * 55)

    # ── Test 1: Tampilkan peta 
    print("\n[TEST 1] Tampilkan peta jaringan RS")
    print("─" * 55)
    g = GraphRujukan()
    g.tampilkan_peta()

    # ── Test 2: BFS — RS terdekat tersedia langsung (1 hop) 
    print("\n[TEST 2] BFS — semua RS tersedia, harusnya temukan 1 hop")
    print("─" * 55)
    g2 = GraphRujukan()
    hasil = g2.bfs_cari_rujukan("Smart Hospital")
    print(f"  RS Tujuan : {hasil['rs_tujuan']}")
    print(f"  Rute      : {' → '.join(hasil['rute'])}")
    print(f"  Hop       : {hasil['hop']}")
    assert hasil["rs_tujuan"] is not None
    assert hasil["hop"] == 1, f"GAGAL: Harusnya 1 hop, dapat {hasil['hop']}"
    print("  ✅ RS terdekat ditemukan dalam 1 hop")

    # ── Test 3: BFS — RS terdekat penuh, cari yang lebih jauh 
    print("\n[TEST 3] BFS — RS 1 hop semua penuh, cari 2 hop")
    print("─" * 55)
    g3 = GraphRujukan()
    # Penuhkan semua RS 1 hop dari Smart Hospital
    g3.set_status("RS Medika", "Penuh")
    g3.set_status("RS Bunda",  "Penuh")
    g3.set_status("RS Kasih",  "Penuh")
    hasil3 = g3.bfs_cari_rujukan("Smart Hospital")
    print(f"  RS Tujuan : {hasil3['rs_tujuan']}")
    print(f"  Rute      : {' → '.join(hasil3['rute'])}")
    print(f"  Hop       : {hasil3['hop']}")
    assert hasil3["hop"] == 2, f"GAGAL: Harusnya 2 hop, dapat {hasil3['hop']}"
    print("  ✅ BFS menemukan RS di 2 hop setelah 1 hop semua penuh")

    # ── Test 4: BFS — semua RS penuh ─────────────────────────────────────
    print("\n[TEST 4] BFS — semua RS penuh → return None")
    print("─" * 55)
    g4 = GraphRujukan()
    for rs in g4.graph:
        g4.set_status(rs, "Penuh")
    hasil4 = g4.bfs_cari_rujukan("Smart Hospital")
    print(f"  RS Tujuan : {hasil4['rs_tujuan']}")
    assert hasil4["rs_tujuan"] is None
    print("  ✅ Mengembalikan None saat semua RS penuh")

    # ── Test 5: BFS dengan langkah — demo presentasi ─────────────────────
    print("\n[TEST 5] BFS dengan langkah (mode demo presentasi)")
    print("─" * 55)
    g5 = GraphRujukan()
    g5.set_status("RS Medika", "Penuh")
    g5.set_status("RS Bunda",  "Penuh")
    hasil5 = g5.bfs_dengan_langkah("Smart Hospital")
    assert hasil5["rs_tujuan"] is not None
    print("  ✅ BFS dengan langkah selesai")

    # ── Test 6: Hanya 1 RS tersisa yang tersedia ──────────────────────────
    print("\n[TEST 6] Hanya RS Sejahtera yang tersedia")
    print("─" * 55)
    g6 = GraphRujukan()
    g6.set_status("RS Medika",   "Penuh")
    g6.set_status("RS Bunda",    "Penuh")
    g6.set_status("RS Kasih",    "Penuh")
    g6.set_status("RS Harapan",  "Penuh")
    hasil6 = g6.bfs_cari_rujukan("Smart Hospital")
    print(f"  RS Tujuan : {hasil6['rs_tujuan']}")
    print(f"  Rute      : {' → '.join(hasil6['rute'])}")
    assert hasil6["rs_tujuan"] == "RS Sejahtera"
    print("  ✅ RS Sejahtera berhasil ditemukan meski paling jauh")

    # ── Test 7: Validasi set_status dengan nama tidak ada ────────────────
    print("\n[TEST 7] set_status dengan nama RS yang tidak ada")
    print("─" * 55)
    g7 = GraphRujukan()
    ok = g7.set_status("RS Fiktif", "Penuh")
    assert ok == False
    print("  ✅ Validasi nama RS tidak ada berjalan benar")
