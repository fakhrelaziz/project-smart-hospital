...
if __name__ == "__main__":
    print("=" * 55)
    print("  TEST MANDIRI — modules/tree_katalog.py")
    print("=" * 55)

    katalog = KatalogObat()

    # Test 1: Tampilkan seluruh tree
    print("\n[TEST 1] Tampilkan seluruh hierarki katalog")
    print("─" * 55)
    katalog.tampilkan()

    # Test 2: Daftar kategori
    print("\n[TEST 2] Daftar semua kategori")
    print("─" * 55)
    kat = katalog.daftar_kategori()
    print(f"  Jumlah kategori: {len(kat)}")
    for k in kat:
        print(f"    → {k}")
    assert len(kat) == 4, f"GAGAL: Harusnya 4 kategori, dapat {len(kat)}"
    print("  ✅ Kategori terbaca dengan benar")

    # Test 3: Obat by kategori
    print("\n[TEST 3] Obat dalam kategori 'Obat Dalam'")
    print("─" * 55)
    obat_dalam = katalog.obat_by_kategori("Obat Dalam")
    for o in obat_dalam:
        print(f"  → {o['nama']} [{o['kode_obat']}]")
    assert len(obat_dalam) == 2
    print("  ✅ Ditemukan 2 obat dalam kategori Obat Dalam")

    # Test 4: Cari obat by nama
    print("\n[TEST 4] Cari obat 'para'")
    print("─" * 55)
    hasil = katalog.cari_obat("para")
    for h in hasil:
        print(f"  → {h['nama']} [{h['kode_obat']}] | Kategori: {h['kategori']}")
    assert len(hasil) == 1
    assert hasil[0]["kode_obat"] == "OBT001"
    print("  ✅ Paracetamol ditemukan dengan benar")

    # Test 5: Cari obat tidak ada
    print("\n[TEST 5] Cari 'betadine' (tidak ada di data)")
    print("─" * 55)
    hasil5 = katalog.cari_obat("betadine")
    print(f"  Hasil: {hasil5}")
    assert hasil5 == []
    print("  ✅ Mengembalikan list kosong")

    # Test 6: Kategori tidak ada
    print("\n[TEST 6] Kategori 'Obat Luar' (tidak ada di data)")
    print("─" * 55)
    hasil6 = katalog.obat_by_kategori("Obat Luar")
    print(f"  Hasil: {hasil6}")
    assert hasil6 == []
    print("  ✅ Mengembalikan list kosong")
